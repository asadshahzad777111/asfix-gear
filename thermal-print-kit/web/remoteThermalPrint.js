 ...
 * Kit version: imports the standalone printApi + receiptEscPos instead of
 * AsFix internal modules.
 */
import { printApi } from './printApi';
import { buildThermalReceiptText } from './receiptEscPos';
import { getSavedPrinter, isNativePosApp } from './nativePosPrint';

export const PRINT_TARGET_KEY = 'pos_print_target_v1';
...
export async function fetchPrintStations() {
  try {
    const data = await printApi.getPrintStations();
...
export async function enqueueRemotePrintJob({
  ...
  if (!stationOnlineForTarget(stations, target)) {
    return {
      ok: false,
      reason: 'no_station',
      message: 'No print station online — open the POS app on Android with a printer, or run the laptop bridge',
      stations,
    };
  }

  try {
    const data = await printApi.createPrintJob({
      text: receiptText,
    ...
    return { ok: false, reason: 'enqueue_failed', message: err?.message || 'Could not send print job' };
  }
}
